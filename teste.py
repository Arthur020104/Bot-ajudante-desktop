...
try:
    try:
        db = sqlite3.connect(f"C:/Usuários/{login}/Documents/Db_userinfo_R2ajudante/r2ajudanteUsuáriosinfo.db")
    except sqlite3.OperationalError:
        db = sqlite3.connect(f"C:/Users/{login}/Documents/Db_userinfo_R2ajudante/r2ajudanteUsuáriosinfo.db")
except sqlite3.OperationalError:
    print("Nao existe")
    try:
        os.mkdir(f"C:/Usuários/{login}/Documents/Db_userinfo_R2ajudante")
    except:
        os.mkdir(f"C:/Users/{login}/Documents/Db_userinfo_R2ajudante")
    try:
        arquivo = open(f"C:/Usuários/{login}/Documents/Db_userinfo_R2ajudante/r2ajudanteUsuáriosinfo.db", 'w+')
    except :
        arquivo = open(f"C:/Users/{login}/Documents/Db_userinfo_R2ajudante/r2ajudanteUsuáriosinfo.db", 'w+')
    arquivo.close()
    db = sqlite3.connect(f"C:/Users/{login}/Documents/Db_userinfo_R2ajudante/r2ajudanteUsuáriosinfo.db")
...
motor = pyttsx3.init()
motor.say(f"{'Bom dia' if now.hour < 12 and now.hour > 4  else 'Boa tarde' if now.hour >= 12 and now.hour < 18 else 'Boa noite'} {login}.")
motor.runAndWait()
def escutar_audio_mic_reconhecer_falar(question = 0, resposta = ''):
    ...
    try:
        ...
        if f'desligar computador {login}'.lower() in frase.lower():
...
def criar_pastawallpapers_senaoexite():
    ...
    if not os.path.exists(f"{path}Wallpapers") :
        try:
            os.mkdir(f"{path}Wallpapers")
        except:
            os.mkdir(f"{path}Wallpapers/")
    if not os.path.exists(f"{path}Ler.txt"):
        try:
            arquivo = open(f"{path}Wallpapers/Ler.txt", 'w+')
        except :
            arquivo = open(f"{path}Wallpapers/Ler.txt", 'w+')
...
def alterar_desktop_img(name):
    ...
    if '.' in name:
        ...
        ctypes.windll.user32.SystemParametersInfoW(win32con.SPI_SETDESKWALLPAPER,0,f"{path}Wallpapers/{name.lower().lstrip()}",changed)
    elif os.path.exists(f"{path}Wallpapers/{name.lower().lstrip()}.png"):
        changed = win32con.SPIF_UPDATEINIFILE | win32con.SPIF_SENDCHANGE
        ctypes.windll.user32.SystemParametersInfoW(win32con.SPI_SETDESKWALLPAPER,0,f"{path}Wallpapers/{name.lower().lstrip()}.png",changed)
    elif os.path.exists(f"{path}Wallpapers/{name.lower().lstrip()}.jpg"):
        changed = win32con.SPIF_UPDATEINIFILE | win32con.SPIF_SENDCHANGE
        ctypes.windll.user32.SystemParametersInfoW(win32con.SPI_SETDESKWALLPAPER,0,f"{path}Wallpapers/{name.lower().lstrip()}.jpg",changed)
    else:
...
def open_app(app, ignorarfala = 0):
    try:
        try:
            os.startfile(f"{path}{app.lstrip()}")
        except:
            files = [f for f in os.listdir(path)]
            nome = str(difflib.get_close_matches(app, files)[0])
            os.startfile(f"{path}{nome}")
        if ignorarfala == 0:
        ...
        try:
            try:
                ...
                files = [f for f in os.listdir("C:/Users/Public/Desktop")]
                nome = str(difflib.get_close_matches(app, files)[0])
                os.startfile(f"C:/Users/Public/Desktop/{nome}")
            if ignorarfala == 0:
                motor.say(f"Abrindo {app.lstrip()}.")
                motor.runAndWait()
        except :
            try:
                try:
                    os.startfile(f"C:/Users/{login}/Desktop/{app.lstrip()}")
                except:
                    files = [f for f in os.listdir(f"C:/Users/{login}/Desktop/")]
                    nome = str(difflib.get_close_matches(app, files)[0])
                    
                    os.startfile(f"C:/Users/{login}/Desktop/{nome}")
                if ignorarfala == 0:
                    motor.say(f"Abrindo {app.lstrip()}.")
                    motor.runAndWait()
            except :
...
def mododejogo(frase):
    ...
    if name == "criar":
        ...
        list_imgs = os.listdir(f"{path}Wallpapers")
        ...
        list_desktop_apps = os.listdir(path)
    ...
    modos_de_jogo = list([str(nome[0]) for nome in modos_de_jogo])
    name = str((difflib.get_close_matches(name, modos_de_jogo,n=1)[0]))
    for i in range(len(modos_de_jogo)):
        print(f"{i+1}.{modos_de_jogo[i]}")
